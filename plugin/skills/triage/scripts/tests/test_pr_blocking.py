...
def test_apply_pr_blocking_credits_pr_closing_a_blocker_of_a_fully_blocked_issue() -> (
    ...
    """A PR that closes one blocker of a fully-blocked backlog issue must be
    credited with blocking that issue, even though the issue itself is
    excluded from the display buckets because another blocker still has no
    open PR (this reproduces the bios repo #159/#219-#225 bug)."""
    backlog_issues = [
        {
            "number": 1,
            "_blocked_by_items": [
