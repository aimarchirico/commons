...
present so a re-run reads as a summary in which nothing changed. Commands whose
output later steps consume emit it for the caller to chain onward.
...
graph LR
    ...
    subgraph Commands["Commons provisioning commands"]
        ...
        expo["commons-expo<br/>create-keystore"]
    end
