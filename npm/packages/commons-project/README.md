...
provisioning packages depend on.

## Install

```bash
pnpm add @aimarchirico/commons-project
```

The one command it exposes takes no arguments, reads its inputs from
`process.env`, fails fast naming every missing variable at once, and is
idempotent: a second run makes no destructive change and reports a no-op.
...
- `replacements` replace each value's literal variants across the matched
  ...
  `lower`, `kebab`, `snake`, `camel`, `pascal`, `title`, and `path` (a
  dotted identifier to path segments). An unknown transform fails with it
  named.
- `moves` interpolate `{{value}}`, `{{value|transform}}`, and
  `{{value.from|transform}}`. An unqualified placeholder takes the side of
  the field it appears in, so a move's `from` reads source values and its
  `to` reads target values.
- Re-running a completed rename finds nothing to change and reports a
  no-op, since the source values are gone.

`rename-project` runs after `pnpm install` on a freshly generated
repository: the replacement scan ignores `node_modules`, so installing
first is safe, and it means the template's own lockfile pins the version of
the tool that rewrites it. Re-run `pnpm install` afterwards when the
manifest moved workspace directories or renamed packages, since both
invalidate the links and `importers` entries recorded at install time.
...
Vitest enforces an 80% line/function/branch/statement coverage floor;
`bin/` entrypoints are excluded.
